...
function Cow (db, key, opts) {
  if (!(this instanceof Cow)) return new Cow(db, key, opts);
  if (!opts) opts = {};
  this._db = db.db || db;
  this._key = key;
  this._prebatch = opts.prebatch || function (rows, cb) { cb(null, rows) };
}
...
Cow.prototype._batch = function (rows, opts, cb) {
  ...
  rows.forEach(function (row) {
    ...
    if (iserr(dkey)) return tick(cb, dkey);
  ...
  self._prebatch(ops, function (err, xops) {
    if (err) cb(err)
    else self._db.batch(xops, opts, cb);
  });
};
